...
def rslidar_callback(msg):
    ...
    if(len(select_boxs)>0):
        ...
        pred_dict['name'] = np.delete(pred_dict['name'], temp)
 
        for i in range(len(pred_dict['name'])):
            if pred_dict['name'][i] == 'Car':
                temp2.append(i)
        only_human_pred = np.delete(pred_dict['name'], temp2)
        only_human_boxs = np.delete(np.array(select_boxs), temp2, 0)
        temp2 = []
        for i in range(len(only_human_pred)):
            if pred_dict['name'][i] == 'Cyclist':
                temp2.append(i)
        only_human_pred = np.delete(only_human_pred ,temp2)
        only_human_boxs = np.delete(only_human_boxs, temp2, 0)
        
        proc_1.pub_rviz.publish_3dbox(only_human_boxs, -1, only_human_pred)
        print_str = f"Frame id: {frame}. Prediction results: \n"
        #for i in range(len(only_human_pred)):
        #    print_str += f"Type: {only_human_pred[i]:.3s} Prob: {only_human_boxs[i]:.2f}\n"
        #print(print_str)
    else:
        aaa = 1 + 1
